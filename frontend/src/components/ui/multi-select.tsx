...
interface MultiSelectProps
  ...
  /**
   * If true, allows selecting/deselecting entire groups at once.
   * Only works with grouped options.
   * Optional, defaults to false.
   */
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const getGroupSelectionState = (group: MultiSelectGroup) => {
      const enabledOptions = group.options.filter((option) => !option.disabled);
      const selectedOptionsInGroup = enabledOptions.filter((option) =>
        selectedValues.includes(option.value),
      );

      if (selectedOptionsInGroup.length === 0) return "none";
      if (selectedOptionsInGroup.length === enabledOptions.length) return "all";
      return "partial";
    };

    const toggleGroup = (group: MultiSelectGroup) => {
      if (disabled || !enableGroupSelection) return;

      const enabledOptions = group.options.filter((option) => !option.disabled);
      const groupValues = enabledOptions.map((option) => option.value);
      const isGroupFullySelected = groupValues.every((value) => selectedValues.includes(value));
      ...
      if (isGroupFullySelected) {
        ...
        // Add all group options to selection (remove duplicates)
        const currentNonGroupValues = selectedValues.filter(
          (value) => !groupValues.includes(value),
        );
        newSelectedValues = [...currentNonGroupValues, ...groupValues];
      }
    ...
    React.useEffect(() => {
      ...
      if (selectedCount !== prevSelectedCount.current) {
        ...
        if (diff > 0) {
          ...
          if (addedLabels.length === 1) {
            ...
            // Check if this might be a group selection
            if (enableGroupSelection && isGroupedOptions(options)) {
              const groupOptions = options as MultiSelectGroup[];
              const addedGroup = groupOptions.find((group) => {
                const groupValues = group.options
                  .filter((opt) => !opt.disabled)
                  .map((opt) => opt.value);
                return (
                  groupValues.every((val) => addedItems.includes(val)) &&
                  groupValues.length === addedItems.length
                );
              });

              if (addedGroup) {
                announce(
                  `Group "${addedGroup.heading}" selected. ${selectedCount} of ${totalOptions} total selected.`,
                );
              } else {
                announce(
          ...
          if (removedCount === 1) {
            ...
            // Check if this might be a group deselection
            if (enableGroupSelection && isGroupedOptions(options)) {
              announce(
                `${removedCount} options removed. ${selectedCount} of ${totalOptions} total selected.`,
              );
            } else {
              announce(
                `${removedCount} options removed. ${selectedCount} of ${totalOptions} total selected.`,
      ...
      if (isPopoverOpen !== prevIsOpen.current) {
        if (isPopoverOpen) {
          const groupInfo =
            enableGroupSelection && isGroupedOptions(options)
              ? ` Groups can be selected by clicking on group headers.`
              : "";
          announce(
            `Dropdown opened. ${totalOptions} options available. Use arrow keys to navigate.${groupInfo}`,
          );
        } else {
              ...
              >
                ...
                {isGroupedOptions(filteredOptions) ? (
                  filteredOptions.map((group) => {
                    const groupSelectionState = enableGroupSelection
                      ...
                      <CommandGroup key={group.heading} heading={group.heading}>
                        {enableGroupSelection && (
                          <CommandItem
                            ...
                            role="option"
                            aria-selected={groupSelectionState === "all"}
                            aria-label={`${group.heading} group${
                            ...
                            className="cursor-pointer font-medium border-b border-border/50 mb-1"
                          >
                            <div
                              className={cn(
                                "mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary",
                            ...
                            >
                              <CheckIcon
                                className={cn(
                                  "h-4 w-4",
                                  groupSelectionState === "partial" && "opacity-70",
                                )}
                              />
                            </div>
                            <span className="flex items-center gap-2">
                              <span>Select all in {group.heading}</span>
                              {groupSelectionState === "partial" && (
                                <Badge variant="secondary" className="text-xs px-1.5 py-0">
                                  ...
                                  /{group.options.filter((opt) => !opt.disabled).length}
                                </Badge>
                              )}
                            </span>
                          </CommandItem>
